...
export default function WalletPage({ user, update }) {
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 28, paddingBottom: 60, backgroundColor: 'transparent' }} showsVerticalScrollIndicator={false}>
      ...
      <View style={{ flexDirection: stackLayout ? 'column' : 'row', gap: 28, alignItems: 'flex-start' }}>
        <View style={{ flex: 1 }}>
          {/* Balance Card */}
          <View style={{ 
            backgroundColor: 'rgba(15,15,26,0.6)', borderRadius: 16, alignItems: 'center', padding: 40, borderWidth: 1.5, borderColor: Colors.gold + '30', marginBottom: 24, overflow: 'hidden', position: 'relative'}}>
            ...
            <Text style={{ color: Colors.emerald, fontFamily: 'Share Tech Mono', fontWeight: '800', fontSize: 22, marginTop: 12 }}>≈ ₹{(((user?.coins ?? 0) / 1000) * 10).toFixed(2)}</Text>
          </View>
            ...
            {tiers.map((t, i) => {
              ...
              return (
                <TouchableOpacity key={i} style={{
                  backgroundColor: 'rgba(15,15,26,0.6)', borderRadius: 14, padding: 20, borderWidth: 1.5,
                  ...
                  <Text style={{ color: ok ? Colors.emerald : Colors.textMuted, fontFamily: 'Share Tech Mono', fontSize: 32, fontWeight: '900' }}>₹{t.r}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
